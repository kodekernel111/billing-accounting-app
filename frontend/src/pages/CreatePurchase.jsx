...
import { createBill } from '../features/purchase/purchaseSlice';
...
const CreatePurchase = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if(!billData.partyId) return alert("Select Supplier");

        await dispatch(createBill({ 
            billData: { ...billData, items: lineItems }, 
    ...
    return (
        <div className="bg-white p-6 rounded shadow">
            <h1 className="text-3xl font-bold mb-6">Enter Purchase Bill</h1>
            <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-3 gap-4 mb-6">
                    <div>
                        <label className="block text-sm font-bold">Bill Number</label>
                        <input className="w-full border p-2 rounded" value={billData.billNumber} onChange={e => setBillData({...billData, billNumber: e.target.value})} required />
                    </div>
                    <div>
                        <label className="block text-sm font-bold">Date</label>
                        <input type="date" className="w-full border p-2 rounded" value={billData.date} onChange={e => setBillData({...billData, date: e.target.value})} required />
                    </div>
                    <div>
                        <label className="block text-sm font-bold">Supplier</label>
                        <select className="w-full border p-2 rounded" value={billData.partyId} onChange={e => setBillData({...billData, partyId: e.target.value})} required>
                ...
                <div className="flex justify-end gap-4 items-center">
                    ...
                    <button type="submit" className="bg-orange-600 text-white px-6 py-2 rounded hover:bg-orange-700">Save Bill</button>
                </div>
            </form>
        </div>
    );
